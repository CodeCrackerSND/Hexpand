...
void test(void) {
	...
	while (t[i].type != 0) {
		type = EVP_get_digestbyname(t[i].type);
		func = extend_get_funcbyname(t[i].type);

		block_size = hash_extend(type, func, t[i].sig, t[i].message, t[i].length, md_value, &tmp);
		output= malloc(2*block_size);
